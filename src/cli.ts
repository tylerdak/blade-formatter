...
export default async function cli() {
  ...
  const parsed = await yargs(hideBin(process.argv))
    ...
    .option("no-condition-whitespace", {
      type: "boolean",
      description:
        "If set to true, no space is added between the condition and parentheses in condition-related directives.\ne.g. @if(true) =/=> @if (true)",
      default: false,
    })
    .option("condition-whitespace", {
      type: "boolean",
      description: "this is a workaround for combine strict && boolean option",
      hidden: true,
      default: true,
  ...
  const options = _.chain(parsed.argv)
    ...
    .set("noConditionWhitespace", !parsed.argv.conditionWhitespace)
    .value();

  ...
  private async preserveConditions(content: string): Promise<any> {
    ...
    return _.replace(
      ...
      (_match: any, p1: any, p2: any, p3: any) => {
        const noConditionWhitespace = this.formatter.options.noConditionWhitespace ?? false;
        if (noConditionWhitespace) {
          return `${p1}(${this.storeConditions(p3)})`;
        }
        return `${p1}${p2}(${this.storeConditions(p3)})`;
      }
      ,
    );
  }
